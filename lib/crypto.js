...
module.exports.decrypt = function decrypt (cipher, nonce, key, callback) {
  key = Buffer.from(key)
  ...
  const err = result ? null : new Error('Unable to decrypt')
  if (callback) {
    err ? callback(err) : callback(err, plaintext)
  } else {
